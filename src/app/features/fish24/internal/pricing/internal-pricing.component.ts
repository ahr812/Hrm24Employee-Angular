    ...
    <div class="mx-auto max-w-[95%] space-y-4 animate-fade-in-up sm:space-y-5" dir="rtl">
      ...
      <section class="rounded-2xl border border-border bg-surface shadow-sm dark:border-slate-700 dark:bg-slate-800" aria-labelledby="pricing-list-title">
        <div class="flex flex-col gap-3 border-b border-border p-3 dark:border-slate-700 sm:flex-row sm:items-center sm:justify-between sm:p-5">
          <div><h2 id="pricing-list-title" class="text-lg font-extrabold text-foreground dark:text-slate-100">لیست قیمت‌گذاری ({{ formatNumber(allRows().length) }} رکورد)</h2><p class="mt-1 text-xs text-muted">مبالغ به ریال هستند؛ تغییر معتبر، رسیدهای داخلی وابسته را بازمحاسبه می‌کند.</p></div>
...
export class InternalPricingComponent {
  ...
  readonly allRows = this.pricing.pricingSettings;
  readonly columnFilteredRows = computed(() => filterByInternalListColumns(this.allRows(), this.columns, this.selectedColumnIds(), this.columnSearches()));
  ...
  exportRows(): void { const columns = this.visibleColumns(); const rows = this.allRows().map(row => columns.map(column => column.exportValue ? column.exportValue(row) : String(column.value(row) ?? ''))); this.xlsx.export('fish24-pricing-settings.xlsx', columns.map(column => column.label), rows); this.toast.show(`${this.formatNumber(rows.length)} رکورد برای Excel آماده شد.`, 'success'); }
