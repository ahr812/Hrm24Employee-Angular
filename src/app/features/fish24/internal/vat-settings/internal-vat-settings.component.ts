    ...
    <div class="mx-auto max-w-[95%] space-y-4 animate-fade-in-up sm:space-y-5" dir="rtl">
      ...
      <section class="rounded-2xl border border-border bg-surface shadow-sm dark:border-slate-700 dark:bg-slate-800" aria-labelledby="vat-list-title">
        <div class="flex flex-col gap-3 border-b border-border p-3 dark:border-slate-700 sm:flex-row sm:items-center sm:justify-between sm:p-5">
          <div><h2 id="vat-list-title" class="text-lg font-extrabold text-foreground dark:text-slate-100">تنظیمات ارزش افزوده ({{ formatNumber(filteredRows().length) }} رکورد)</h2><p class="mt-1 text-xs text-muted">مبلغ و ارزش افزوده هر فاکتور هنگام صدور ثبت می‌شود و تغییر تنظیمات، فاکتورهای صادرشده را تغییر نمی‌دهد.</p></div>
      ...
      @if (toggleTarget(); as row) {
        <div appEscToClose (escPressed)="cancelToggle()" class="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 p-3 backdrop-blur-sm" (click)="cancelToggle()"><section role="alertdialog" aria-modal="true" aria-labelledby="vat-toggle-title" class="w-full max-w-md rounded-2xl border border-border bg-surface p-4 shadow-2xl dark:border-slate-700 dark:bg-slate-800" (click)="$event.stopPropagation()"><h2 id="vat-toggle-title" class="text-lg font-extrabold text-foreground dark:text-slate-100">تأیید {{ row.isActive ? 'غیرفعال‌سازی' : 'فعال‌سازی' }}</h2><p class="mt-2 text-sm leading-7 text-muted">این تغییر فقط وضعیت تنظیم را عوض می‌کند و مبلغ یا ارزش افزوده فاکتورهای صادرشده را تغییر نمی‌دهد. آیا ادامه می‌دهید؟</p>@if (conflictError()) {<p role="alert" class="mt-3 rounded-xl bg-danger/10 p-3 text-sm font-bold text-danger">{{ conflictError() }}</p>}<div class="mt-5 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end"><button type="button" (click)="cancelToggle()" class="min-h-11 rounded-xl border border-border px-4 text-sm font-bold text-foreground dark:border-slate-600 dark:text-slate-200">انصراف</button><button type="button" (click)="confirmToggle()" class="min-h-11 rounded-xl bg-primary px-5 text-sm font-bold text-white">تأیید</button></div></section></div>
      }

      @if (deleteTarget(); as row) {
        <div appEscToClose (escPressed)="cancelDelete()" class="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 p-3 backdrop-blur-sm" (click)="cancelDelete()"><section role="alertdialog" aria-modal="true" aria-labelledby="vat-delete-title" class="w-full max-w-md rounded-2xl border border-border bg-surface p-4 shadow-2xl dark:border-slate-700 dark:bg-slate-800" (click)="$event.stopPropagation()"><h2 id="vat-delete-title" class="text-lg font-extrabold text-foreground dark:text-slate-100">حذف تنظیم ارزش افزوده</h2><p class="mt-2 text-sm leading-7 text-muted">آیا از حذف تنظیم «{{ row.title }}» مطمئن هستید؟ تنظیمی که در بازه فعلی آن فاکتور صادر شده باشد قابل حذف نیست.</p>@if (deleteError()) {<p role="alert" class="mt-3 rounded-xl bg-danger/10 p-3 text-sm font-bold text-danger">{{ deleteError() }}</p>}<div class="mt-5 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end"><button type="button" (click)="cancelDelete()" class="min-h-11 rounded-xl border border-border px-4 text-sm font-bold text-foreground dark:border-slate-600 dark:text-slate-200">انصراف</button><button type="button" (click)="confirmDelete()" class="min-h-11 rounded-xl bg-danger px-5 text-sm font-bold text-white">حذف</button></div></section></div>
      }
    </div>
  `,
  styles: [`
...
export class InternalVatSettingsComponent {
  ...
  readonly deleteTarget = computed(() => this.financial.vatSettings().find(row => row.id === this.deleteId()) ?? null);
  ...
  save(event: Event): void { event.preventDefault(); const result = this.financial.saveVatSetting(this.draft(), this.editingId()); this.fieldErrors.set(result.fieldErrors); if (result.protectedSetting) { this.conflictError.set(`ویرایش این تنظیم ممکن نیست؛ در بازه فعلی ${result.protectedSetting.startDate} تا ${result.protectedSetting.endDate} فاکتور صادر شده است. فاکتور صادرشده قابل بازمحاسبه نیست.`); return; } if (result.conflict) { this.conflictError.set(`این بازه با تنظیم فعال «${result.conflict.title}» از ${result.conflict.startDate} تا ${result.conflict.endDate} هم‌پوشانی دارد. هیچ تغییری اعمال نشد.`); return; } if (!result.ok) return; this.closeForm(); this.toast.show('تنظیمات ارزش افزوده ذخیره شد؛ فاکتورهای صادرشده بدون تغییر باقی ماندند.', 'success'); }
  ...
  confirmToggle(): void { const row = this.toggleTarget(); if (!row) return; const result = this.financial.toggleVatSetting(row.id); if (result.conflict) { this.conflictError.set(`فعال‌سازی ممکن نیست؛ بازه با تنظیم فعال «${result.conflict.title}» هم‌پوشانی دارد. هیچ تغییری اعمال نشد.`); return; } this.cancelToggle(); this.toast.show('وضعیت تنظیم تغییر کرد؛ فاکتورهای صادرشده بدون تغییر باقی ماندند.', 'success'); }
  requestDelete(row: VatSettingRecord): void { this.deleteId.set(row.id); this.deleteError.set(''); }
  cancelDelete(): void { this.deleteId.set(null); this.deleteError.set(''); }
  confirmDelete(): void { const row = this.deleteTarget(); if (!row) return; const result = this.financial.deleteVatSetting(row.id); if (result.protectedSetting) { this.deleteError.set(`حذف ممکن نیست؛ در بازه ${row.startDate} تا ${row.endDate} فاکتور صادر شده است.`); return; } if (!result.ok) { this.deleteError.set('حذف تنظیم انجام نشد.'); return; } this.cancelDelete(); this.toast.show('تنظیم ارزش افزوده حذف شد؛ فاکتورهای صادرشده بدون تغییر باقی ماندند.', 'success'); }
