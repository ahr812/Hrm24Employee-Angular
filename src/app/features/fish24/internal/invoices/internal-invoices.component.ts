...
@Component({
  ...
  <div class="mx-auto max-w-[95%] space-y-4 animate-fade-in-up sm:space-y-5" dir="rtl">
    ...
    <section class="card overflow-hidden" aria-labelledby="invoice-list-title">
      <div class="flex flex-col gap-3 border-b border-border p-3 dark:border-slate-700 sm:p-5"><div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><h2 id="invoice-list-title" class="text-lg font-extrabold">لیست فاکتورها ({{ number(filteredRows().length) }} رکورد)</h2><div class="flex flex-wrap gap-2"><button type="button" (click)="columnChooserOpen.set(!columnChooserOpen())" class="toolbar"><ui-icon name="sliders" [size]="16"></ui-icon>انتخاب ستون</button><button type="button" (click)="exportRows()" class="toolbar text-success"><ui-icon name="download" [size]="16"></ui-icon>Excel</button></div></div><div class="flex flex-wrap gap-2" aria-label="خروجی‌های تخصصی فاکتورها"><button type="button" (click)="exportInvoices()" class="toolbar text-success"><ui-icon name="download" [size]="16"></ui-icon>خروجی فاکتور</button><button type="button" (click)="exportAccountingVoucher()" class="toolbar text-success"><ui-icon name="download" [size]="16"></ui-icon>خروجی سند حسابداری</button><button type="button" (click)="exportCustomers()" class="toolbar text-success"><ui-icon name="download" [size]="16"></ui-icon>خروجی تفصیلی مشتریان</button></div></div>
    ...
    @if (smsInvoice()) {<div appEscToClose (escPressed)="closeSms()" class="modal" (click)="closeSms()"><section role="dialog" aria-modal="true" class="dialog max-w-lg" (click)="$event.stopPropagation()"><div class="dialog-head"><h2 class="text-lg font-extrabold">ارسال پیامک مستقیم</h2><button type="button" (click)="closeSms()" aria-label="بستن"><ui-icon name="x" [size]="19"></ui-icon></button></div><div class="space-y-3 p-4"><p class="text-sm text-muted">گیرنده: {{ smsInvoice()!.name || 'نام ناموجود' }} — <span dir="ltr">{{ smsInvoice()!.mobile }}</span></p><textarea [(ngModel)]="smsText" rows="4" class="input h-auto py-3" aria-label="متن پیامک"></textarea><button type="button" class="primary" (click)="sendSms()">بررسی ارسال</button></div></section></div>}
...
export class InternalInvoicesComponent {
  ...
  private readonly xlsx = inject(PlainXlsxService); private readonly toast = inject(ToastService); private readonly router = inject(Router); private readonly users = inject(BusinessUserPreviewService);
  private readonly rolePreview = inject(Fish24RolePreviewService); private readonly smsHistory = inject(InternalSmsHistoryPreviewService);
  ...
  readonly smsInvoice = signal<InternalFormalInvoiceListItem | null>(null); smsText = ''; private smsSubmissionKey = ''; readonly unavailableInvoice = signal<InternalFormalInvoiceListItem | null>(null); readonly pendingDelete = signal<InternalFormalInvoiceListItem | null>(null);
  ...
  exportAccountingVoucher(): void { const result = buildAccountingVoucherExport(this.filteredRows()); this.xlsx.export('fish24-accounting-vouchers.xlsx', ACCOUNTING_VOUCHER_HEADERS, result.rows); const warning = result.incompatibleTrackingSources.length ? `؛ شناسه پیگیری غیرعددی ${this.number(result.incompatibleTrackingSources.length)} فاکتور فقط در شرح حفظ شد.` : '.'; this.toast.show(`${this.number(result.exportedInvoiceCount)} فاکتور در سند حسابداری آماده شد${warning}`, 'success'); }
  exportCustomers(): void { const rows = buildCustomerDetailExportRows(this.filteredRows(), employerId => this.users.findUser(Number(employerId))); this.xlsx.export('fish24-customer-details.xlsx', CUSTOMER_DETAIL_EXPORT_HEADERS, rows); this.toast.show(`${this.number(rows.length)} مشتری یکتا با اطلاعات جاری پروفایل آماده شد.`, 'success'); }
  ...
  openSms(row: InternalFormalInvoiceListItem): void { this.smsText = ''; this.smsSubmissionKey = this.smsHistory.nextSubmissionKey('invoice-direct'); this.smsInvoice.set(row); }
  closeSms(): void { this.smsInvoice.set(null); this.smsSubmissionKey = ''; }
  sendSms(): void {
    const invoice = this.smsInvoice(); const message = this.smsText.trim(); const submissionKey = this.smsSubmissionKey;
    if (!invoice || !message || !submissionKey) { this.toast.show('متن پیامک نمی‌تواند خالی باشد.', 'error'); return; }
    const recipient = this.users.findUser(Number(invoice.employerId));
    if (recipient) this.smsHistory.recordCompletedMockSend({ recipient, message, senderRole: this.rolePreview.getPreviewRole(), source: 'invoice-direct', submissionKey });
    this.toast.show('ارسال واقعی انجام نشد؛ پیامک مستقیم فقط در پیش‌نمایش بررسی شد.', 'success'); this.closeSms();
  }
