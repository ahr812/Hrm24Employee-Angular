...
export class InternalInvoicesComponent {
  ...
  readonly deferredExports = ['خروجی تفصیلی مشتریان'] as const;
  ...
  exportAccountingVoucher(): void { const result = buildAccountingVoucherExport(this.filteredRows()); this.xlsx.export('fish24-accounting-vouchers.xlsx', ACCOUNTING_VOUCHER_HEADERS, result.rows); const warnings = [result.excludedUnknownSources.length ? `${this.number(result.excludedUnknownSources.length)} فاکتور با منشأ نامشخص کنار گذاشته شد` : '', result.incompatibleTrackingSources.length ? `شناسه پیگیری غیرعددی ${this.number(result.incompatibleTrackingSources.length)} فاکتور فقط در شرح حفظ شد` : ''].filter(Boolean).join('؛ '); this.toast.show(`${this.number(result.exportedInvoiceCount)} فاکتور در سند حسابداری آماده شد${warnings ? `؛ ${warnings}.` : '.'}`, 'success'); }
