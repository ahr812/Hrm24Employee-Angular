...
@Component({
  ...
  standalone: true,
  imports: [FormsModule, RouterLink, IconComponent, TicketAttachmentPickerComponent],
  template: `
        ...
        <section class="rounded-2xl border border-border bg-surface p-4 shadow-sm dark:border-slate-700 dark:bg-slate-800 sm:p-6" aria-labelledby="ticket-request-title">
          ...
          <form (ngSubmit)="submit()" novalidate>
            <div class="space-y-4">
              ...
              <div><span class="block text-sm font-bold">پیوست‌ها</span><app-ticket-attachment-picker inputId="ticket-create-files" [files]="files()" [errorMessage]="fileError()" (filesSelected)="selectFiles($event)" (removeFile)="removeFile($event)"></app-ticket-attachment-picker></div>
            </div>
...
export class EmployerTicketCreateComponent {
  ...
  selectFiles(selected: readonly File[]): void { const next = [...this.files(), ...selected]; const error = this.preview.validateAttachments(next); this.fileError.set(error ?? ''); if (!error) this.files.set(next); }
